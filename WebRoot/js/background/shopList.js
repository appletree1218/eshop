	...
	deleteValidate : function() {
		...
		if (index == 0) {
			...
			for (var i = 0; i < addresses.length-1; i++) {
				del_href = del_href + "shopId"+i+"=" +addresses[i];
			...
			if (del.deleteConfirm()) {
				$("#delete").attr("href", "shopAction_deleteShop?"+del_href);
...
var update = {
	updateValidate : function() {
		...
		if (index == 0) {
			...
			$("#update").attr("href","shopAction_updateUI?shopId="+id);
...
var skip = {
	skipPage : function(){
		$("#s").bind("change",function(){
			var num = $(this).value;
			window.location.href="shopAction_getShopList?pageNum="+num;
		});
	}
};
	...
	validateShop : function(){
		...
		$("#status").bind("focusout",function(){
			...
			if($(this).val()==2){
				...
				$.ajax({
					type:"POST",
					url:"ajaxShopAction_updateShopStatus.action",
					data:param,
