	...
	deleteValidate : function() {
		...
		if (index == 0) {
			...
			for (var i = 0; i < addresses.length-1; i++) {
				del_href = del_href + "userId"+i+"=" +addresses[i];
			...
			if (del.deleteConfirm()) {
				$("#delete").attr("href", "userAction_deleteUser?"+del_href);
...
var update = {
	updateValidate : function() {
		...
		if (index == 0) {
			...
			$("#update").attr("href","userAction_updateUI?userId="+id);
...
var skip = {
	skipPage : function(){
		$("#jump").bind("change",function(){
			var num = $(this).val();
			window.location.href="userAction_getUserList?pageNum="+num;
		});
	}
};
